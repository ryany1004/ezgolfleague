class CourseTeeBoxesController < BaseController
  ...
  def create
    ...
    if @course_tee_box.save
      ...
      { success: 'The tee box was successfully created. Add the hole info next.' }
    else
      render :new
    end
  end
  ...
  def update
    if @course_tee_box.update(course_tee_box_params)
      ...
      { success: 'The course tee box was successfully updated.' }
    else
      render :edit
    end
  end
  ...
  def destroy
    ...
    redirect_to course_course_tee_boxes_path(@course), flash:
    { success: 'The course tee box was successfully deleted.' }
  end

  private

  def course_tee_box_params
    params.require(:course_tee_box).permit(:name,
                                           :rating,
                                           :slope,
                                           :tee_box_gender)
  end
