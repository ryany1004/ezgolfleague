class Play::TournamentsController < Play::BaseController
  ...
  def leaderboard
    ...
    @day_flights_with_rankings = fetch_flights_with_rankings(@tournament_day)
    @combined_flights_with_rankings = fetch_combined_flights_with_rankings(@tournament_day, @day_flights_with_rankings)
  ...
  def confirm
    ...
    { success: 'You are confirmed for the tournament.' }
  end
  ...
  def complete_signup
    if @tournament.includes_player?(current_user)
      redirect_to play_tournament_signup_path(@tournament), flash:
      { error: 'You are already registered for this tournament. Remove your existing registration and try again.' }
    else
      ...
      if paying_now == true
        ...
        redirect_to play_dashboard_index_path, flash:
        { success: 'You are registered for the tournament.' }
      end
    end
  end
