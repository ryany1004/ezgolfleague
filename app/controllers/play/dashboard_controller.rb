class Play::DashboardController < Play::BaseController
  ...
  def index
    if current_user.selected_league.blank? && current_user.impersonatable_users.blank?
      ...
      { success: 'Please create or join a league to continue.' }
    elsif current_user.selected_league.blank? && current_user.impersonatable_users.present?
      render 'select_user'
    else
      ...
      if @league_season == active_season
        ...
        @todays_tournaments.each do |t|
          @has_unpaid_upcoming_tournaments = false unless t.user_has_paid?(current_user)
        end
      end

      @upcoming_tournaments = Tournament.all_upcoming([current_user.selected_league], @league_season.ends_at).select(&:all_days_are_playable?).to_a
      @past_tournaments = Tournament.past_for_league_season(@league_season).select(&:all_days_are_playable?).to_a
