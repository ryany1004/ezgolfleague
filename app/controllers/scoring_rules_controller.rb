class ScoringRulesController < BaseController
  ...
  after_action :update_primary_scoring_rule, only: [:update, :create]
  ...
  def update
    ...
    @tournament_day.tournament_groups.each(&:create_daily_teams) if @scoring_rule.team_type == ScoringRuleTeamType::DAILY && @tournament_day.daily_teams.count.zero?
  ...
  def set_primary
    @tournament_day.scoring_rules.find(params[:scoring_rule_id]).update(primary_rule: true)

    redirect_to league_tournament_tournament_day_scoring_rules_path(@tournament.league, @tournament, @tournament_day)
  end

  def update_primary_scoring_rule
    return unless @tournament_day.scorecard_base_scoring_rule.blank? && @tournament_day.scoring_rules.first.present?
