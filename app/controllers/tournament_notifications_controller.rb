class TournamentNotificationsController < BaseController
  ...
  def new
    ...
    @notification_template.deliver_at = Time.zone.now - 5.minutes # always in the past
  ...
  def destroy
    ...
    redirect_to league_tournament_tournament_day_tournament_notifications_path(@tournament.league, @tournament, @tournament_day), flash:
    { success: 'The notification was successfully deleted.' }
  end

  private

  def notification_template_params
    params.require(:notification_template).permit(:title,
                                                  :body,
                                                  :deliver_at,
                                                  :tournament_notification_action,
                                                  :tournament_id,
                                                  :league_id)
  end
