class TournamentNotificationsController < BaseController
  ...
  def create
    ...
    if @notification_template.save
      ...
      if params[:commit] == "Save & Complete Tournament Setup"
        ...
        redirect_to league_tournament_tournament_day_tournament_notifications_path(@tournament.league, @tournament, @tournament_day), flash: { success: "The notification was successfully created." }
      end
    else
      render :new
    end
  ...
  def update
    if @notification_template.update(notification_template_params)
      ...
      redirect_to league_tournament_tournament_day_tournament_notifications_path(@tournament.league, @tournament, @tournament_day), flash: { success: "The notification was successfully updated." }
    else
      render :edit
    end
  end
  ...
  def destroy
    ...
    redirect_to league_tournament_tournament_day_tournament_notifications_path(@tournament.league, @tournament, @tournament_day), flash: { success: "The notification was successfully deleted." }
  end
