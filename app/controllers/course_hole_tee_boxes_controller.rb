class CourseHoleTeeBoxesController < BaseController
  ...
  def create
    ...
    if @course_hole_tee_box.save      
      redirect_to course_course_hole_course_hole_tee_boxes_path(@course, @course_hole), :flash => { :success => "The tee box was successfully created." }
    else            
      render :new
    end
  end
  ...
  def destroy
    ...
    redirect_to course_course_hole_course_hole_tee_boxes_path(@course, @course_hole), :flash => { :success => "The tee box was successfully deleted." }
  end
