  ...
  def index
    if @current_user.leagues.blank?
      ...
      @tournaments = Rails.cache.fetch(cache_key, expires_in: 8.hours, race_condition_ttl: 10) do
        ...
        todays_tournaments = Tournament.all_today(@current_user.leagues).includes(:league, tournament_days: [:course, :scoring_rules])
        upcoming_tournaments = Tournament.all_upcoming(@current_user.leagues, nil).includes(:league, tournament_days: [:course, :scoring_rules])
        ...
        tournaments = tournaments.select { |t| t.all_days_are_playable? }.to_a
  ...
  def results
    ...
    @tournament_results = Rails.cache.fetch(cache_key, expires_in: 24.hours, race_condition_ttl: 10) do
      tournament.tournament_days.each do |d|
        ...
        tournament_presenter = TournamentPresenter.new({ tournament: tournament,
                                                         tournament_day: d,
                                                         user: current_user,
        ...
        @tournament_results << { payouts: payouts,
                                 rankings: rankings,
                                 contests: optional_scoring_rules_with_dues }
      end
