class ReportsController < BaseController
  ...
  def index
    ...
    @past_tournaments = Tournament.tournaments_happening_at_some_point(nil, nil, leagues, true)
  ...
  def finalization_report
    @past_tournaments = Tournament.tournaments_happening_at_some_point(nil, nil, nil, true)
                                  .where(is_finalized: true).reorder(tournament_starts_at: :desc)
                                  .page(params[:page]).without_count
  end
  ...
  def confirmed_players
    ...
    @eager_groups = TournamentGroup.includes(golf_outings: [{ scorecard: :scores }, :user])
                                   .where(tournament_day: @tournament_day)
                                   .order(:tee_time_at)
  end

  def fetch_tournament_day
    @tournament = fetch_tournament_from_user_for_tournament_id(params[:tournament])
