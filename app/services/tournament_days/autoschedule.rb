module TournamentDays
  module Autoschedule
    def schedule_golfers
      previous_day = tournament.previous_day_for_day(self)
      return if previous_day.blank?

      tournament.tournament_days.first.assign_users_to_flights(should_destroy_results: false)
      ...
      tournament.players.each do |p|
        ...
        if result.blank? # this is a hack for Best Ball # TODO FIX ME
          result_name = ResultName.result_name_for_user(p, previous_day.scorecard_base_scoring_rule)
      ...
      if tournament.auto_schedule_for_multi_day == AutoScheduleType::AUTOMATIC_WORST_FIRST # worst golfer, worst flight
      ...
      elsif tournament.auto_schedule_for_multi_day == AutoScheduleType::AUTOMATIC_BEST_FIRST # best golfer, best flight
      ...
      Rails.logger.info { "Auto-Scheduling For Day: #{id}" }
      ...
      tournament_groups.each do |group|
        ...
        next unless tournament.display_teams?
      ...
      players_with_scores.each_with_index do |player_score, index|
        ...
        if group_slots.count > index
          ...
          existing_group = tournament_group_for_player(player)

          if existing_group.present?
            Rails.logger.info { "Removing #{player.complete_name} from existing group #{existing_group.id}." }

            remove_player_from_group(tournament_group: existing_group, user: player, remove_from_teams: true)
          ...
          add_player_to_group(tournament_group: slot, user: player, paying_with_credit_card: false, confirmed: true, registered_by: 'Auto-Schedule')
