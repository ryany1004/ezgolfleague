class TournamentPresenter
  ...
  def day_has_tournament_teams?
    self.tournament_day.blank? ? false : self.tournament_day.tournament_teams.count > 0
  end
  ...
  def tournament_players
    ...
    if self.day_has_tournament_teams?
      teams = []

      self.tournament_day.tournament_teams.each do |tournament_team|
        group = tournament_team.users.blank? ? nil : self.tournament_day.tournament_group_for_player(tournament_team.users.first)

        teams << {name_data: tournament_team, group: group, id: nil} unless group.blank?
      end
